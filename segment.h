...
/**
 * 内存中分配的段条目，集合了各个映射表,可以与磁盘上的f2fs_sit_entry相比，多出了一些checkpoint相关的映射表
 * checkpoint时，valid_blocks以及cur_valid_map的值会被写入到ckpt_valid_blocks和ckpt_valid_map
 * **/
struct seg_entry {
...
struct free_segmap_info {
	...
	unsigned long *free_segmap;	/* free segment bitmap ,注意 1为 dirty使用过？,0为free*/
 ...
 * 相当于每一个log的实际分配管理
 * 主要用于确定系统free block的分配信息，进行管理
 * 记录着当前segment的分配信息
 * 关联到f2fs_sm_info中的curseg_info数组
*/
struct curseg_info {
		...
		unsigned int segno)
{
	...
	spin_lock(&free_i->segmap_lock);
	//设置这一位，如果之前已经设置过了则不会扣free_segments
...
{	
	...
	//检查计算得出的valid blocks与f2fs_sit_entry中的vblocks 低10位是否相同
						...
						unsigned int start)
{
	 ...
	 * 如果所属块在sit map上offset处被置位了，则该f2fs_sit_block在后面的sit区域副本
	 * 确定在哪个部分，也就是由于双副本而产生的偏移*/
	if (f2fs_test_bit(offset, sit_i->sit_bitmap))
