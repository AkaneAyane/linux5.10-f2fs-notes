...
/**
 * SIT管理结构 segment manager
 * 管理着CP中的active segment信息
 * 是一个跨区域的管理结构
 * 使用segment.c中的build_segment_manager函数初始化
*/
struct f2fs_sm_info {
	struct sit_info *sit_info;				/* whole segment information 所有的segment信息*/
	struct free_segmap_info *free_info;		/* free segment information 空闲segment信息*/
	struct dirty_seglist_info *dirty_info;	/* dirty segment information 脏segment信息*/
	
	/** active segment information 
	 * 这里管理着F2FS的6个curseg_info，每一个log对应着一个curseg_info
	*/
...
/**
 * 这是f2fs的superblock在内存中的结构
 * 除了包含superblock相关的信息还包含很多其他的管理结构，
 * 包括锁,SIT,NAT结构
 * 初始化过程定义在super.c中的:init_sb_info函数
 * 
*/
struct f2fs_sb_info {	
	struct super_block *sb;			/* pointer to VFS super block */
	struct proc_dir_entry *s_proc;		/* proc entry */
	struct f2fs_super_block *raw_super;	/* raw super block pointer 直接从磁盘中读取到的super_block*/
...
//从ckpt中获取nat_ver_bitmap或者sit_ver_bitmap
static inline void *__bitmap_ptr(struct f2fs_sb_info *sbi, int flag)
{
	struct f2fs_checkpoint *ckpt = F2FS_CKPT(sbi);
	int offset;
	//判定是否设置了CP_LARGE_NAT_BITMAP_FLAG
	...
	//一般情况走此分支，首先判定cp_paylod的情况，如果>0，那么存在额外空间
	if (__cp_payload(sbi) > 0) {
		//nat部分从cppack最后一个字节开始
		if (flag == NAT_BITMAP)
